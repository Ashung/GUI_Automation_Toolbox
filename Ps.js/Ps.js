...
function imageSize(width, height, options) {
    ...
    if(typeof(traversalLayes) == "function") {
        traversal(app.activeDocument, function() {
            if(activeDocument.activeLayer.kind == (LayerKind.SOLIDFILL || LayerKind.GRADIENTFILL || LayerKind.PATTERNFILL)
                && activeDocument.activeLayer.vectorMaskFeather != 0) {
                    // Feather: 0 - 1000
                    var feather = activeDocument.activeLayer.vectorMaskFeather * scale;
                    activeDocument.activeLayer.vectorMaskFeather = feather > 1000 ? 1000 : feather;;
            }
        });
    }
}
